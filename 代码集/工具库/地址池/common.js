...
cc.Class({
    ...
    initNodePool: function (that, objInfo) {
        ...
        cc.vv.commonState.poolObj[poolName] = that[poolName];
        // 创建对象，并放入池中
    ...
    clearAllPool: function () {
        for (const key in cc.vv.commonState.poolObj) {
            if (cc.vv.commonState.poolObj.hasOwnProperty(key)) {
                const pool = cc.vv.commonState.poolObj[key];
                pool.clear();
